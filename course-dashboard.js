...
/* Lets the next /course page load skip the guest view before Firebase resolves.
   The guest markup is server-rendered, so without this flag a returning user
   would see it flash before the dashboard replaces it. */
function rememberSignedIn(signedIn) {
  try {
    if (signedIn) localStorage.setItem('cd-signed-in', '1');
    else localStorage.removeItem('cd-signed-in');
  } catch (e) { /* private mode - the flash is the only cost */ }
}

function showGuestView() {
  document.documentElement.classList.remove('cd-returning');
  ...
  enhanceGuestCurriculum();
}

/* The guest curriculum ships in the HTML (generated by generate-course-links.py,
   between the static-course markers) with every unit open so it works without JS.
   The inline script on course/index.html normally wires up the accordion before
   first paint; this is the backstop for when that script did not run. */
function enhanceGuestCurriculum() {
  const container = document.getElementById('cdGuestUnits');
  if (!container || container.dataset.enhanced === '1') return;

  const units = container.querySelectorAll('.cd-unit');
  if (!units.length) {
    renderGuestCurriculum(); // markers missing or block empty - build from data
    return;
  }
...
/* Fallback only - see enhanceGuestCurriculum above. */
...
async function loadDashboard(user) {
  ...
  // Swap the server-rendered guest view for the dashboard
  document.getElementById('cdGuest').classList.add('hidden');
  document.getElementById('cdGuestSticky').classList.add('hidden');
  document.documentElement.classList.remove('cd-returning');
  document.getElementById('cdLoading').classList.add('hidden');
